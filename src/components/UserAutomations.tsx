...
const UserAutomations = () => {
  ...
  return (
    <div className="mx-auto mt-5 w-11/12 h-[45%] border-4 border-blue-600">{userAutomations.data?.length ? userAutomations.data?.map(((automation, i) => (
    ...
    ))) : userAutomations.isLoading ? <div>loading...</div> : <div>Waiting for your first creation!</div>}</div>
  )
}
